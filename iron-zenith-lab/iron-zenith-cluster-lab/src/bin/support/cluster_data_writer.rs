...
pub(crate) async fn write_current_node_cluster_data(
    ...
    for write_index in 1..=3 {
        ...
        let value = format!("{node_name}|{node_addr}|{node_role}|write={write_index}");
        ...
        if let Err(error) = result {
            eprintln!(
                "[Iron] [cluster-lab] 集群业务数据写入失败 node_id={node_id} node_name={node_name} write_index={write_index} error={error}"
            );
        }
