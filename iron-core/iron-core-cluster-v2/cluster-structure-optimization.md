...
| 原则 | 说明 |
|---|---|
| 入口少 | `IronRaftClusterManager::run()` 只保留一条主线。 |
| 阶段稳定 | 启动过程固定为 6 个阶段，不轻易增减阶段。 |
...
| 3 | `spawn_runtime_services` | 启动 Raft TCP、调试 HTTP、leader 维护后台任务。 |
| 4 | `bootstrap_or_join_cluster` | 先尝试加入已有集群；只有唯一起盘节点允许初始化新集群。 |
| 5 | `join_remaining_boot_nodes` | 起盘成功后，把其他注册节点逐个加入为 voter。 |
| 6 | `serve_forever` | 保持进程运行，持续承载 Raft 服务。 |

## 当前文件职责

| 文件 | 职责 |
|---|---|
| `iron_raft_cluster_manager.rs` | 管理器入口，只负责编排启动主线和保存当前节点上下文。 |
...
| 判断 | 建议 |
|---|---|
| `Flow` 是否继续拆文件 | 暂不拆。当前 6 阶段清晰，继续拆会增加跳转成本。 |
...
| 是否新增启动仲裁结构 | 不新增。固定 `is_boot_node = true` 已满足当前生产约定。 |
...
- P0 TCP 保护已经集中在 frame/server 层，后续调参优先改 `iron_raft_constants.rs`。
