...
impl IronRaftClusterManager {
    ...
    pub async fn run(self) -> Result<(), Box<dyn Error>> {
        ...
        if let Some(http_debug_addr) = debug_http_addr {
            ...
            tokio::spawn(async move {
                if let Err(error) =
                    start_query_http_with_addr(node_id, node_name, http_debug_addr, query_raft).await
                {
    ...
    fn validate_topology(&self) -> Result<(), Box<dyn Error>> {
        ...
        if self.boot_nodes.values().any(|node| !node.is_boot_node()) {
            return Err(IoError::new(ErrorKind::InvalidInput, "boot_nodes 中的节点必须都是 Boot 角色").into());
        }

        let contains_current = self.boot_nodes.contains_key(&self.current_node.node_id);
        if contains_current != self.current_node.is_boot_node() {
            return Err(
                IoError::new(
    ...
    async fn bootstrap_or_join_cluster(
        ...
        let self_tag = self_node_tag(self.current_node.node_id, &self.current_node.node_name);
        let many_tag = many_nodes_tag(self.boot_nodes.iter().filter_map(|(peer_id, peer)| {
        ...
        let is_boot_node = self.current_node.is_boot_node();
        ...
        loop {
            ...
            // 4.3 如果当前节点不是 boot 节点，就继续等待集群可加入。
            if !is_boot_node {
    ...
    // 如果当前节点是 bootstrap owner，就把其他节点逐个加进集群。
    async fn join_remaining_nodes(
        &self,
        raft: &Raft<IronRaftTypeConfig>,
