...
pub(crate) fn run_registry_cluster_from_local_toml() -> Result<(), ClusterError> {
    ...
    let debug_http_addr =
        read_env_or_string("IRON_REGISTRY_HTTP_ADDR", &seed_config.debug_http.http_addr);
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()?;

    runtime.block_on(crate::core::runtime::start_registry_cluster(
        cluster_id,
...
pub(crate) fn run_worker_from_local_toml(biz_kind: BizServiceKind) -> Result<(), ClusterError> {
    ...
    runtime.block_on(crate::core::runtime::start_worker(
        cluster_id,
        biz_kind,
        biz_service_id,
        seed_config.registry_nodes,
