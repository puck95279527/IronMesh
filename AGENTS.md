...
- 本节规范适用于所有 workspace 子 crate，包括 `iron-gateway`、`iron-business/*`、`iron-core/*`、`iron-protocol/*` 和 `iron-zenith-lab/*`。
...
- 根目录 `[workspace.dependencies]` 必须按能力类别细分，并使用中文注释标明类别，例如 Web 服务依赖、异步运行时依赖、协议与序列化依赖、日志与可观测性依赖、workspace 内部 crate。
- 根目录 `[workspace.dependencies]` 中启用特殊 feature 的依赖，必须在依赖附近用中文注释说明 feature 目的，避免子 crate 私自拆分 feature。
- 根目录 `[workspace.dependencies]` 中固定精确版本号的依赖，必须在依赖附近用中文注释说明固定原因，例如与生成工具链、协议代码或兼容性策略绑定。
...
- 子目录中的 `AGENTS.md` 不得放宽本节 Cargo Workspace 规范；如需补充，只能增加更严格的约束。
